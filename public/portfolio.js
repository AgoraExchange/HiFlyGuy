...
export function createPortfolio(host) {
  const root = document.createElement('section'); root.id = 'portfolio'; root.hidden = true; root.setAttribute('aria-label', 'Aster Wallet portfolio');
  root.innerHTML = `<div class="wallet-head"><span class="wallet-emblem" aria-hidden="true">&#9671;</span><div class="wallet-wordmark">ASTER WALLET<small>FLYGUY / PERSONAL PORTFOLIO</small></div><span class="wallet-currency" title="Portfolio currency">USD</span><button class="wallet-exit" id="wallet-exit" aria-label="Return to charts">&#8599;</button></div>
  ...
  function chart(symbol) {
    ...
    if (points.length < 2) return `<div class="wallet-chart"><div class="wallet-chart-empty">${fetching || historyBusy ? 'Loading market history' : 'Price history unavailable'}</div></div><div class="wallet-chart-note"><span>7D / DAILY QUOTES</span><span>Coinbase market history</span></div>`;
    ...
    return `<div class="wallet-chart"><svg viewBox="0 0 400 90" preserveAspectRatio="none" role="img" aria-label="${symbol || 'Portfolio'} valuation from public daily quotes"><defs><linearGradient id="wallet-fill" x1="0" y1="0" x2="0" y2="1"><stop stop-color="${color}" stop-opacity=".25"/><stop offset="1" stop-color="${color}" stop-opacity="0"/></linearGradient></defs><path d="M0 27H400 M0 53H400 M0 80H400" stroke="#2c4233" stroke-width=".5" stroke-dasharray="2 5"/><path d="${path} L394 90 L6 90Z" fill="url(#wallet-fill)"/><path d="${path}" fill="none" stroke="${color}" stroke-width="2" vector-effect="non-scaling-stroke"/><circle cx="394" cy="${coords.at(-1).split(',')[1]}" r="3" fill="${color}"/></svg></div><div class="wallet-chart-note"><span>${symbol ? '7D / DAILY SPOT PRICES' : '7D / PORTFOLIO VALUE'}</span><span>${points[0] ? new Date(points[0].at).toLocaleDateString('en-US',{month:'short',day:'numeric',timeZone:'UTC'}) : ''} &mdash; latest</span></div>`;
  }
  ...
  function render() {
    ...
    if (view === 'allocation') {
      ...
      content = `<div class="wallet-eyebrow">Portfolio composition</div><div class="wallet-allocation"><div class="wallet-ring" style="background:conic-gradient(${gradient})" role="img" aria-label="Portfolio allocation"></div><div class="wallet-legend">${assets.map(a => `<div><i style="background:${a.color}"></i>${a.name}<b>${total ? (a.value / total * 100).toFixed(1) + '%' : '&mdash;'}</b></div>`).join('')}</div></div><div class="wallet-cash"><span>Total portfolio value</span><strong>${usd(total)}</strong></div>`;
    } else if (view === 'activity') {
      content = `<div class="wallet-eyebrow">This desk session</div><div class="wallet-log">${activity.slice(0,5).map(a => `<div class="wallet-log-row"><i>&#8599;</i><span>${a.text}</span><time>${new Date(a.at).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false})}</time></div>`).join('') || '<div class="wallet-log-empty">Your portfolio visits appear here.</div>'}</div><div class="wallet-chart-note">Portfolio activity</div>`;
    } else if (view === 'detail') {
      const h = HOLDINGS.find(h => h.symbol === selected);
      content = `<div class="wallet-eyebrow">${h.name} / ${h.symbol}-USD</div><div class="wallet-balance">${usd(quotes[selected]?.price)}</div>${change(selected)}${chart(selected)}<div class="wallet-detail-meta"><span>Your position<strong>${h.quantity} ${h.symbol}</strong></span><span>Position value<strong>${usd(quotes[selected] ? Math.round(quotes[selected].price * h.quantity * 100) / 100 : null)}</strong></span></div>`;
    } else {
      content = `<div class="wallet-eyebrow">Total portfolio value</div><div class="wallet-balance" id="wallet-total">${usd(total)}</div>${change()}${chart()}<div class="wallet-chart-note" style="margin-top:13px">Portfolio value / USD</div>`;
    }
  ...
  function show(visible) {
    if (visible !== !root.hidden) {
      ...
      if (visible) { view = 'overview'; root.scrollTop = 0; record('Opened portfolio'); render(); void refresh(); }
    }
  }
