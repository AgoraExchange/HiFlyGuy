...
test('Tesla action plays to completion, replays, and leaves the saved world untouched',async({page})=>{
  ...
  const before=await page.evaluate(()=>localStorage.getItem('hiflyguy.world.v1'));
  ...
  const after=await page.evaluate(()=>localStorage.getItem('hiflyguy.world.v1'));const a=JSON.parse(before),b=JSON.parse(after);delete a.savedAt;delete b.savedAt;expect(b).toEqual(a);
