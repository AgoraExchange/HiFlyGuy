...
export function setupDirector({ viewport, prepare }) {
  ...
  panel.innerHTML=`<div class="director-panel-head"><span>DIRECTORS MODE / ACTIONS</span><button id="director-close" aria-label="Close actions">&times;</button></div><h2 id="director-title">Give him a scene.</h2><p>One action. Every camera move. Ready for your edit.</p><button id="maybach-action" class="director-action"><span class="director-action-art" aria-hidden="true">28<span>PH</span></span><span><strong>MMM MAYBACH MUSIC</strong><small>Elevator &rarr; apartment &rarr; Aster Wallet &rarr; chair lean</small><em>2 SEC LEAD-IN &nbsp; / &nbsp; 22 SEC TAKE</em></span><b aria-hidden="true">&#8599;</b></button><p class="director-instructions">Start your screen recording first. Controls disappear, then the scene starts after two seconds. Double-tap the scene or press Escape to stop.</p><p class="director-audio">Silent take &middot; Add your sound in Instagram or TikTok.</p><p id="director-error" role="status" hidden></p>`;
  ...
  function ensureStage(){if(!stage)stage=new DirectorStage(frame);return stage;}
  ...
  $('#director-close').onclick=()=>panel.close();$('#maybach-action').onclick=start;
  ...
  return {
    ...
    update(){
      ...
      if(elapsed>=LEAD_IN+TAKE_DURATION)complete();
    },
  };
}
