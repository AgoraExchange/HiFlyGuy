...
export function setupDirector({ viewport, prepare }) {
  ...
  panel.innerHTML=`<div class="director-panel-head"><span>DIRECTORS MODE / ACTIONS</span><button id="director-close" aria-label="Close actions">&times;</button></div><h2 id="director-title">Give him a scene.</h2><p>One action. Every camera move. Ready for your edit.</p><button id="maybach-action" class="director-action"><span class="director-action-art" aria-hidden="true">28<span>PH</span></span><span><strong>MMM MAYBACH MUSIC</strong><small>Elevator &rarr; apartment &rarr; Aster Wallet &rarr; chair lean</small><em>2 SEC LEAD-IN &nbsp; / &nbsp; 22 SEC TAKE</em></span><b aria-hidden="true">&#8599;</b></button><button id="lab-action" class="director-action director-action-lab"><span class="director-action-art" aria-hidden="true">04<span>LAB</span></span><span><strong>PROFIT IS MANDATORY</strong><small>Losing trade &rarr; isolation &rarr; interrogation &rarr; electric correction</small><em>2 SEC LEAD-IN &nbsp; / &nbsp; 24 SEC TAKE</em></span><b aria-hidden="true">&#8599;</b></button><button id="comeback-action" class="director-action director-action-comeback"><span class="director-action-art" aria-hidden="true">06<span>FEET</span></span><span><strong>SIX FEET. THREE PAIRS.</strong><small>Lab comeback &rarr; profit &rarr; elevator fit &rarr; six sandals &rarr; feet on desk</small><em>2 SEC LEAD-IN &nbsp; / &nbsp; 18 SEC TAKE</em></span><b aria-hidden="true">&#8599;</b></button><p class="director-instructions">Start your screen recording first. Controls disappear, then the scene starts after two seconds. Double-tap the scene or press Escape to stop.</p><p class="director-audio">Silent takes &middot; Comeback: LV Sandals by EsDeeKid, fakemink &amp; Rico Ace. Lab: Locrian Dominant by Aloboi. Add your sound in your editor.</p><p id="director-error" role="status" hidden></p>`;
  ...
  function ensureStage(){
    ...
    if(!stage){stage=selected==='comeback'?new ComebackStage(frame):selected==='lab'?new LabStage(frame):new DirectorStage(frame);stageKind=selected;}
    return stage;
  }
  ...
  return {
    ...
    update(){
      ...
      if(elapsed>=LEAD_IN+(selected==='comeback'?COMEBACK_DURATION:selected==='lab'?LAB_DURATION:TAKE_DURATION))complete();
    },
  };
}
