...
import { DestroyStage } from './director-destroy-stage.js';
import { DESTROY_VOICEOVER } from './director-destroy-timeline.js';
...
export function setupDirector({ viewport, prepare, authorize=()=>true, allowed=()=>true }) {
  ...
  function ensureStage(){
    ...
    if(!stage){stage=selected==='destroy'?new DestroyStage(frame):selected==='opponent'?new OpponentStage(frame):selected==='tesla'?new TeslaStage(frame):selected==='comeback'?new ComebackStage(frame):selected==='lab'?new LabStage(frame):new DirectorStage(frame);stageKind=selected;}
    return stage;
  }
  ...
  function start(){
    ...
    if(selected==='destroy')stage.configure({drop:parseFloat($('#destroy-drop').value)});
  ...
  $('#destroy-action').onclick=()=>{selected='destroy';start();};
